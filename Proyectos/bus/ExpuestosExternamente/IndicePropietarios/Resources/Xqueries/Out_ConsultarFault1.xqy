...
declare function local:func($codigo as xs:string, $mensaje as xs:string) as element()  (:: schema-element(ns1:salidaIndicePropietarios)::) {
    <ns1:salidaIndicePropietarios>
        ...
        {
        if($codigo = '500')
        then <ns1:codMensaje>493</ns1:codMensaje>
        else(<ns1:codMensaje>{fn:data($codigo)}</ns1:codMensaje>)
        }
        {
        if($codigo = '500')
        then <ns1:descripcionMensaje>El campo tipoIdentificacionPredio es un campo opcional, pero al venir dato en este campo, solo se acepta "matricula", "numeroPredial", "numeroPredialAnterior" o "NUPRE</ns1:descripcionMensaje>
        else(<ns1:descripcionMensaje>{fn:data($mensaje)}</ns1:descripcionMensaje>)
        }
        
        
        <ns1:numeroConsulta></ns1:numeroConsulta>
    </ns1:salidaIndicePropietarios>
};
